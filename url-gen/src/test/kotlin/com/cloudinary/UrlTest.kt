...
class UrlTest {
    ...
    fun testSecureDistribution() { // should use default secure distribution if secure=TRUE
        val result = cloudinary.url(secure = true).generate("test")
    ...
    fun testSecureDistributionOverwrite() { // should allow overwriting secure distribution if secure=TRUE
        val result =
            cloudinary.url(secure = true, secureDistribution = "something.else.com").generate("test")
    ...
    fun testSecureDistibution() { // should take secure distribution from config if secure=TRUE
        ...
        val result = Cloudinary(newConfig).url(secure = true).generate("test")
...
// secure_distribution
    ...
    fun testFetch() { // should escape fetch urls
        val result = cloudinary.url(type = "fetch").generate("http://blah.com/hello?a=b")
        assertEquals(
    ...
    fun testCname() { // should support external cname
        val result = cloudinary.url(cname = "hello.com").generate("test")
    ...
    fun testSupportUrlSuffixForRawUploads() {
        val actual =
            cloudinary.url(urlSuffix = "hello", privateCdn = true, resourceType = "raw").generate("test")
    ...
    fun testSupportUrlSuffixForVideoUploads() {
        val actual =
            cloudinary.url(urlSuffix = "hello", privateCdn = true, resourceType = "video").generate("test")
    ...
    fun testSupportUseRootPathForPrivateCdn() {
        ...
        actual =
            cloudinary.url(privateCdn = true, transformation = Transformation().rotate { angle(0) }, useRootPath = true)
                .generate("test")
    ...
    fun testHttpEscape() { // should escape http urls
        val result =
            cloudinary.url(type = "youtube").generate("http://www.youtube.com/watch?v=d9NF2edxy-M")
        assertEquals(
    ...
    fun testFoldersWithExcludeVersion() { // should not add version if the user turned off forceVersion
        var result = cloudinary.url(forceVersion = false).generate("folder/test")
    ...
    fun testSignedUrl() { // should correctly sign a url
        ...
        var actual =
            cloudinary.url(
                version = "1234", transformation = Transformation().resize(Resize.crop { width(10); height(20) }),
                signUrl = true
            )
                .generate("image.jpg")
        assertEquals(expected, actual)
        expected = DEFAULT_UPLOAD_PATH + "s----SjmNDA--/v1234/image.jpg"
        actual = cloudinary.url(version = "1234", signUrl = true).generate("image.jpg")
        assertEquals(expected, actual)
        expected = DEFAULT_UPLOAD_PATH + "s--Ai4Znfl3--/c_crop,h_20,w_10/image.jpg"
        actual = cloudinary.url(
            transformation = Transformation().resize(Resize.crop { width(10); height(20) }),
            signUrl = true
        )
            .generate("image.jpg")
        assertEquals(expected, actual)
